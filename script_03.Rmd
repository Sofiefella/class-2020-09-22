...
sep_old <- 
  ...
  group_by(id, title, department, name) %>%
...
```{r}
sep_new <- read_excel("new_data/class_enrollment_summary_by_term_9-21-2020.xlsx", 
                      skip = 3) %>%
  clean_names() %>% 
  filter(! is.na(course_title)) %>% 
  select(-grad, -non_degree, -x_reg, -vus, -employee, 
...
```{r}
# Wisdom would suggest that we need to make smart assumptions which make the world 
# a little easier to comprehend. We have already made some smart assumptions with
# this data set to simplify it, for example we have filtered the data and removed data 
# with NA. If we didn't make assumptions we would have too much complicated data to work 
# with, for example we would have way too many treatments to plot onto a chart and too 
# many variables to account for. Therefore we use wisdom to simplify our data. One of the 
# assumptions we would also make is that the Dean cares about the drop in undergraduate 
# enrollment. 
```
...
```{r}
# Temperance is used to recognize the limitations of our data. These enrollment 
# results are dependent on the time that we are collecting the data. For example, 
# the data changed from last class to this class, so it could change from today to 
# tomorrow as well. There is also variation in individuals that could influence their
# enrollment decisions that we cannot account for in our data. 
```
...
```{r}
enrollment <- inner_join(sep_new, sep_old, by = c("id", "name", "department", "title")) %>%
